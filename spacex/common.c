...
void spacex_populate_fdt_chosen(void *fdt)
{
	...
	fdt_add_uint64_to_chosen(fdt, "uboot-time-ns", get_current_time_ns());
	...
	/*
	 * Add serial number if it exist. Some Starship nodes booting from TFTP
	 * do not use `serialnum`.
	 */
...
/**
 * Init the env vars for uBoot
 *
 */
__weak int spacex_initr_env(void)
{
	return 0;
}
