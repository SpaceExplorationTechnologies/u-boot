...
static void telem_send_packet(const char *buf, int len)
{
	...
	eth = eth_get_dev();
	/*
	 * The interface is not ready but also has been claimed. Trying to
	 * claim or activate it is not safe.
	 */
	if (eth == NULL || eth_is_activating(eth))
		return;
...
static int telem_stdio_start(struct stdio_dev *dev)
{
	 ...
	 */
#ifdef CONFIG_SPACEX_XILINX_SOFT_REBOOT
 ...
 */
static int do_telem_cmd(struct cmd_tbl *cmdtp, int flag, int argc,
			  char * const argv[])
{
