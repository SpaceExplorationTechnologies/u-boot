...
/**
 * Computes the number of nanoseconds corresponding to the number of processor
 * ticks.
 */
static uint64_t ticks_to_ns(unsigned long long ticks)
{
	/*
	 * Note the order of calculation here, so that we avoid
	 * integer division errors; normally one would calculate:
	 *
	 * (NSEC_PER_SEC / get_tbclk()) * ticks
	 *
	 ...
	 * multiplication (NSEC_PER_SEC * ticks) overflows 64
	 * bits.
	 ...
	 */
	return lldiv((NSEC_PER_SEC / 1000) * ticks, get_tbclk() / 1000);
}
			  ...
			  unsigned src, unsigned len)
{
	...
	uint64_t ns_since_boot = ticks_to_ns(response_ticks);
...
/**
 * Retrieves the current time, in nanoseconds, since boot.
 */
uint64_t get_current_time_ns(void)
{
	return ticks_to_ns(get_raw_ticks());
}
