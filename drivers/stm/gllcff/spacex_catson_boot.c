...
#define CATSON_SIP_SVC_GET_CORE_FREQ 0xC2000070
...
void catson_late_init(void)
{
	...
	struct pt_regs regs_pll_settings = {};

	regs_pll_settings.regs[0] = CATSON_SIP_SVC_GET_CORE_FREQ;
	/*
	 * regs_pll_settings[0] -> 0 if successful
	 * regs_pll_settings[1] -> A5x_SS_SSCG_PLL_FREQ_RW
	 * regs_pll_settings[2] -> A5x_SS_SSCG_PLL_SSCG_RW
	 * regs_pll_settings[3] -> A5x_SS_SSCG_PLL_ENABLE_RW
	 */
	smc_call(&regs_pll_settings);

	if (regs_pll_settings.regs[0] == 0) {
		u64 core_frequency = 0;
		u64 ndiv = regs_pll_settings.regs[1] & 0x7f;
		u64 odf = (regs_pll_settings.regs[1]  >> 8) & 0x7;
		u64 idf = (regs_pll_settings.regs[1]  >> 16) & 0x7;
		u64 f_in = 60;
		u64 frac = regs_pll_settings.regs[2] & 0xffff;
		bool is_frac = (regs_pll_settings.regs[3] >> 4) & 0x1;

		if (!is_frac) {
			u64 ldf = 2 * ndiv;

			core_frequency = ((f_in * ldf) / (2 * idf * odf)) / 2;
		} else {
			u64 ldf_adj = 2 * ((ndiv << 17) + (frac << 1) + 1);

			core_frequency = (((f_in * ldf_adj) / (2 * idf * odf)) >> 17) / 2;
		}
		env_set_ulong("core_frequency_mhz", core_frequency);
		printf("ARM Core Frequency: %lld", core_frequency);
	} else {
		printf("SMC call for ARM core frequency failed!");
	}
	...
	env_set("bootargs", CATSON_BOOTARGS_TFTP);
#else
	if (linux_boot_slot == 0) {
		puts("BOOT SLOT A\n");
		snprintf(modeboot, sizeof(modeboot), "%s_a", boot_string);
		env_set("linux_boot_slot", "0");
#ifndef CONFIG_CATSON_EMMC_BOOT
		env_set("bootargs", CATSON_BOOTARGS_A);
#endif
	} else {
		puts("BOOT SLOT B\n");
...
#ifndef CONFIG_CATSON_EMMC_BOOT
		env_set("bootargs", CATSON_BOOTARGS_B);
#endif
	}
#endif
...
void catson_populate_fdt_chosen(void *fdt)
{
	...
	if (!starlink_catson_seed_kaslr(&kaslr_seed)) {
		fdt_add_uint64_to_chosen(fdt, "kaslr-seed", kaslr_seed);
	}
	...
	fdt_add_env_to_chosen(fdt, "core_frequency_mhz", NULL);
}
...
int is_prod_hw(void)
{
	const u32 data = readl(SECURITY_BSEC_PROD_FUSE);
 ...
 */
static int call_sx_is_prod_hw(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
